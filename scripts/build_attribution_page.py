...
def main() -> None:
    ...
    # rendered page silently lost everything after it, including the BKG attribution the source
    # requires. Escape every cell.
    def cell(text: str) -> str:
        return str(text).replace("|", "\\|")

    STANDARD = "Standard citation of the source applies."

    def row(key: str, count: int, with_terms: bool) -> str:
        label = labels.get(key, key)
        url = urls.get(key, "")
        name = f"[{cell(label)}]({url})" if url else cell(label)
        if with_terms:
            return f"| {name} | {count} | {cell(TERMS[key])} |"
        return f"| {name} | {count} |"

    ordered = sorted(counts.items(), key=lambda kv: -kv[1])
    specific = [(k, c) for k, c in ordered if k in TERMS]
    standard = [(k, c) for k, c in ordered if k not in TERMS]
    ...
    # 44 links in one column is a very tall stack; the class sets them in two.
    lines.append("::: {.gl-cols}\n")
    for slug, entry in sorted(registry.items(), key=lambda kv: kv[1]["name"]):
        if entry["url"]:
            lines.append(f"- [{cell(entry['name'])}]({entry['url']})")
    lines.append("\n:::\n")
    ...
    # The same two figures used to be typed by hand on the landing page and on Accessing.qmd and
    # drifted apart (12 299 / 12 497 / 12 501). They now come from here, so one rebuild moves all
    # of them: the pages say {{< var geodb_rows >}} and {{< var geodb_sources >}}.
    variables = out.parent / "_variables.yml"
    variables.write_text(
        "# Generated by destatis-rag/scripts/build_attribution_page.py. Do not edit by hand.\n"
        f"geodb_rows: \"{rows_fmt}\"\n"
